...
export const EmployeeManagement: React.FC = () => {
  const [employees, setEmployees] = useState<User[]>( // State for employees
    getUsers().filter(u => u.role === 'employee') // Filter for 'employee' role
  );
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this employee? This will also delete all their time entries.')) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Správa účtů zaměstnanců a hodinových sazeb
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {employees.reduce((sum, emp) => sum + emp.deductions, 0).toLocaleString()} CZK
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {employees.map((employee) => (
                  <tr key={employee.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {employee.deductions.toLocaleString('cs-CZ')} Kč
                    </td>
